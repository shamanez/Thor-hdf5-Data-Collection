...
import time
import sys
from dagger_constants import ACTION_SIZE, GAMMA, LOCAL_T_MAX, ENTROPY_BETA, VERBOSE, NUM_VAL_EPISODES, VALIDATE, VALIDATE_FREQUENCY, SUCCESS_CUTOFF, MAX_VALID_STEPS
	...
	def save_expert(self):
		...
		for i in range(10):
			...
			while(not(self.env.terminal)):
				...
				action = self.choose_action(oracle_pi) #select the action probabilistically
		...
		self.open_file_and_save('trajectory/actions_history.csv', action_history_list)
		#pdb.set_trace()
