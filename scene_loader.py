...
class THORDiscreteEnvironment(object):  #This is the main environemtns
  ...
  def __init__(self, config=dict()):
    ...
    self.observations=self.h5_file['observation'][()]#300, 400, 3 image for each location
  ...
  def step(self, action):
    ...
    print("Printing the current state ID",self.current_state_id)
    ...
    if self.terminal:
      print("I am in the terminal state")
    #print("pringint thhe new state ID",self.current_state_id)
    #print("Prinintg expert's collided or not:--",self.collided)
    
    #print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")

    #self.s_t[:,1:] This is to making the last element (first element - oldest elemet)  out from the the list 
...
#Here the state ID is the currrent state
  ...
  '''
  def _tiled_state_Images(self, state_id):
#Here the state ID is the currrent state
    
    Img= self.h5_file['observation'][state_id][:,:,:,np.newaxis]#get the resnet feature of the state 
    return np.tile(Img, (1, self.history_length)) #coping the target in to last for frames :) 
  '''
